...
const SkillImage = styled.img`
    ...
    &.active {
        opacity: 1;
        transform: translate(-50%, -50%) scale(1); // Scale in when active
    }

    &.inactive {
        opacity: 0;
        transform: translate(-50%, -50%) scale(0.8); // Scale out when inactive
    }
`;
...
const Skills = () => {
	...
	const getSkillName = (skill, index) => { // Now takes index
        const logo = allLogos[skill][index]; // Get the specific logo
        return logo.name; // Use name or extract from src
    };
            ...
            <SkillsSection id="skills">
                ...
                <SkillsGrid>
                    {Object.keys(skillsData).map((skill) => {
                        ...
                        const currentLogo = allLogos[skill][currentIndex];
                        const level = currentLogo.level;
                        ...
                        return (
                            ...
                            >
                                <SkillProgress bgColor={currentBgColor} skillLevel={level} />
